<template>
    <v-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" max-width="500"
        ...
        <v-card>
            ...
            <v-card-text>
                <v-form>
                    <v-select :items="items" v-model="value" label="Hospital"></v-select>
                    <v-text-field label="Codigo del Departamento"></v-text-field>
                    <v-text-field label="Nombre del Departamento"></v-text-field>
                </v-form>
            </v-card-text>
...
const items = ref(["Unidad 1", "Unidad 2"])
</script>